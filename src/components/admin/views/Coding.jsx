...
const Coding = () => {
    ...
    const loadInitialData = async () => {
        try {
            setLoading(true)
            const [challengesData, employeesData] = await Promise.all([
                fetchChallenges(),
                fetchAdminEmployees()
            ])
            setProblems(Array.isArray(challengesData) ? challengesData : [])
    ...
    const handleAddProblem = async (e) => {
        ...
        const newChallenge = {
            ...
            difficulty,
            status: 'PENDING',
            assignedTo: assignedTo || 'All Personnel',
            solveUrl: 'https://leetcode.com/problems/' + title.toLowerCase().replace(/ /g, '-')
        }

        try {
            await postChallenge(newChallenge)
            loadInitialData() // Refresh list to show all inclusive data
            setTitle('')
            setDesc('')
            setModal({
                show: true,
                title: "Protocol Synchronized",
                message: "Submit successfully: Challenge signal transmitted successfully to selected personnel.",
                type: 'success'
            });
        } catch (error) {
    ...
    return (
        <div className='flex gap-8 h-full text-black bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 relative overflow-hidden'>
            <div className='w-1/3 p-10 bg-purple-50/50 rounded-[3rem] border border-purple-100 h-fit relative'>
                ...
                <div className='flex items-center gap-3 mb-8'>
                    ...
                    <div>
                        <h2 className='text-2xl font-black text-purple-900 tracking-tight'>Coding Program</h2>
                ...
                <form onSubmit={handleAddProblem} className='flex flex-col gap-6 relative z-10'>
                    <div className='space-y-1.5'>
                        <label className='text-[10px] font-black text-purple-600 uppercase tracking-widest ml-1'> Title</label>
                        <input
                            value={title}
                    ...
                    <div className='space-y-1.5'>
                        <label className='text-[10px] font-black text-purple-600 uppercase tracking-widest ml-1'>Assign to Personnel</label>
                        <select
                            value={assignedTo}
                    ...
                    <div className='space-y-1.5'>
                        <label className='text-[10px] font-black text-purple-600 uppercase tracking-widest ml-1'>Description </label>
                        <textarea
                            value={desc}
                    ...
                    >
                        ...
                        {isSubmitting ? 'Syncing Modules...' : 'Submit'}
                    </button>
                </form>
            </div>

            <div className='flex-1 p-10 bg-slate-50/50 rounded-[3rem] border border-slate-100 h-full text-black overflow-y-auto scrollbar-hide'>
                <div className='flex items-center justify-between mb-10'>
                    <div className='flex items-center gap-4'>
                        ...
                        <div>
                            ...
                            <p className='text-slate-400 text-[10px] font-black uppercase tracking-widest mt-0.5'>Code Overview</p>
                        </div>
                    </div>
                ...
                {loading ? (
                    ...
                    <div className='grid gap-4'>
                        {problems.map((prob, idx) => (
                            <div key={prob.id || `prob-${idx}`} className='p-6 bg-white rounded-[2rem] flex justify-between items-center border border-slate-100 shadow-sm hover:border-purple-300 hover:shadow-xl hover:shadow-purple-500/5 transition-all group animate-in slide-in-from-right duration-300'>
                                <div className='flex items-center gap-6'>
                                    ...
                                    <div>
                                        ...
                                        <div className='flex items-center gap-3 mt-1'>
                                            <span className={`text-[9px] font-black uppercase tracking-[0.2em] px-2.5 py-1 rounded-lg ${prob.difficulty === 'HARD' ? 'bg-red-50 text-red-600' :
                                            ...
                                            <span className='text-[9px] text-purple-400 font-black uppercase tracking-widest flex items-center gap-1.5 border-l border-slate-200 pl-3'>
                                                <Users size={10} />
                                                {prob.assignedTo || 'All Personnel'}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div className='p-10 space-y-8'>
                            <div className='grid grid-cols-2 gap-4'>
                                <div className='p-6 bg-slate-50 rounded-[2rem] border border-slate-100'>
                                    ...
                                    <span className={`text-sm font-black uppercase tracking-widest ${selectedProblem.difficulty === 'HARD' ? 'text-red-600' :
                                ...
                                <div className='p-6 bg-purple-50 rounded-[2rem] border border-purple-100 col-span-2'>
                                    <p className='text-[10px] font-black text-purple-400 uppercase tracking-widest mb-1'>Assigned Personnel</p>
                                    <div className='flex items-center gap-2'>
                                        <Users size={16} className='text-purple-600' />
                                        <span className='text-sm font-black tracking-tight text-purple-900 uppercase'>{selectedProblem.assignedTo || 'Universal Broadcast'}</span>
                                    </div>
                                </div>
                            </div>
